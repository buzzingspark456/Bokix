...
export async function resolveInvoiceLineItems({ userId, companyId, invoiceId }) {
  ...
  // vara en fast, orelaterad procentsats) plus Bokix egen marginal ovanpå.
  // Stripes riktiga Sverige-prislista (stripe.com/en-se/pricing) är 1,5% +
  // 1,80 kr för europeiska kort, upp till 3,15% + 1,80 kr för utländska.
  // En SANN dynamisk "Stripes verkliga avgift" känns bara efter att kortet
  // dragits (Platform Pricing Tool, som skulle räknat ut den automatiskt,
  // funkar tyvärr INTE för direct charges på Standard-konton — Stripes
  // egen begränsning, se docs.stripe.com/connect/platform-pricing-tools#
  // requirements) — så det här är en UPPSKATTNING satt när betalningslänken
  // skapas, inte en exakt efterhandsberäkning: antar den vanligaste
  // europeiska kortavgiften (1,5% + 1,80 kr) plus Bokix egen marginal
  // (2% + 0,50 kr), totalt 3,5% + 2,30 kr. Blir något för lågt för
  // utländska kort (som egentligen kostar 3,15% hos Stripe själv), men det
  // är den bästa avvägningen mellan "korrekt branding + pengarna direkt
  // till kunden" (direct charge, kräver ett belopp i förväg) och en exakt
  ...
  const PLATFORM_FEE_PERCENT = Number.parseFloat(process.env.STRIPE_PLATFORM_FEE_PERCENT || '3.5');
  const PLATFORM_FEE_FIXED_ORE = Number.parseInt(process.env.STRIPE_PLATFORM_FEE_FIXED_ORE || '230', 10);
